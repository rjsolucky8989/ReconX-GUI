...
# ── Tiny embedded SVG logo ─────────────────────
_LOGO_SVG = (
    "PHN2ZyB3aWR0aD0iNjQiIGhlaWdodD0iNjQiIHZpZXdCb3g9IjAgMCA2NCA2NCIgZmlsbD0ibm9uZS"
    "IgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48Y2lyY2xlIGN4PSIzMiIgY3k9IjMy"
    "IiByPSIyOCIgZmlsbD0iI0ZGRiIvPjwvc3ZnPg=="
)

def _logo_pixmap() -> QPixmap:
    px = QPixmap()
    px.loadFromData(base64.b64decode(_LOGO_SVG))
    return px
...
class Runner(QThread):
    ...
    def run(self):
        ...
        for idx, mid in enumerate(self.ids, 1):
            ...
            outfile = self.outdir / f"{mid}_{name.replace(' ', '')}.txt"
            self.log.emit(colored(f"\n[+] {name}: {cmd}\n", "cyan"))
            with outfile.open("w", encoding="utf-8", errors="ignore") as fh:
                ...
                for line in proc.stdout:
                    ...
                    fh.write(line)
                proc.wait()
            self.done_one.emit(name, str(outfile))
            self.progress.emit(int(idx / total * 100))
        self.done_all.emit(self.outdir)

# ── Main Application Window ───────────────────
class MainWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("RECONX – v1.7.0")
    ...
    def _build_ui(self):
        ...
        # Header
        hdr = QHBoxLayout()
        lbl_logo = QLabel(); lbl_logo.setPixmap(_logo_pixmap().scaledToHeight(48, Qt.SmoothTransformation))
        ...
        hdr.addWidget(lbl_logo); hdr.addWidget(lbl_title, 1); hdr.addStretch()
        ...
        # Module list
        self.list = QListWidget(); self.list.setSelectionMode(QListWidget.MultiSelection)
        for mid, (name, _) in MODULES.items():
            item = QListWidgetItem(f"[{mid}] {name}"); item.setCheckState(Qt.Checked)
            self.list.addItem(item)
        self.list.setMinimumWidth(260)
        splitter.addWidget(self.list)

        # Right panel
        right = QWidget(); rlayout = QVBoxLayout(right); splitter.addWidget(right)

        # Target row
        trow = QHBoxLayout()
        self.lbl_target = QLabel("<i>Target not set</i>")
        self.lbl_target.setTextInteractionFlags(Qt.TextSelectableByMouse)
        btn_set = QPushButton("Set Target …"); btn_set.clicked.connect(self.set_target)
        self.btn_run = QPushButton("▶ Run"); self.btn_run.clicked.connect(self.on_run)
        trow.addWidget(self.lbl_target, 1); trow.addWidget(btn_set); trow.addWidget(self.btn_run)
        rlayout.addLayout(trow)

        # Log
        self.log = QTextEdit(); self.log.setReadOnly(True)
        self.log.setLineWrapMode(QTextEdit.NoWrap)
        self.log.document().setDefaultFont(QFont("Consolas", 10))  # ✅ Fixed line
    ...
    def choose_outdir(self):
        dir = QFileDialog.getExistingDirectory(self, "Select Output Directory", str(self.output_root))
        if dir:
            self.output_root = Path(dir)
    ...
    def on_run(self):
        ...
        self.runner.done_all.connect(lambda _: QMessageBox.information(self, "Finished", "All modules completed."))
        self.runner.start()
    ...
    def _apply_theme(self):
        self.setStyleSheet(textwrap.dedent("""
            * { background:#1e1e1e; color:#dcdcdc; font-family:Arial,Helvetica,sans-serif; }
            QPushButton {
                background:#333; border:1px solid #444; padding:6px 12px;
            }
            QPushButton:hover { background:#444; }
            QPushButton:disabled { background:#555; color:#888; }
            QTextEdit, QListWidget { background:#121212; }
            QProgressBar {
                background:#121212; border:1px solid #444; text-align:center; }
        """))
